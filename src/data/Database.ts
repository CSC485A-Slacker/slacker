...
import { addPin, removePin, updatePin } from "../redux/PinSlice";
...
class Database implements IDatabase {
  ...
  async getAllUsers(): Promise<IUserActionResult<IUser[]>> {
    try {
      const pinsCollection = collection(this.database, "users").withConverter(
        userConverter
      );
      ...
      return new UserActionResult<IUser[]>(
        new DatabaseActionResult(true, `Succeeded: users retrieved`),
        userList
      );
    } catch (error) {
      return new UserActionResult<IUser[]>(
    ...
    try {
      ...
      if (previousLocation) {
        ...
        if (userIsCheckedIntoSpot(user, newLocation)) {
          ...
          throw new Error(
            `User already checked into spot ${coordinateToString(newLocation)}.`
          );
        }
  ...
  async checkoutFromSpot(userID: string, location: LatLng) {
    console.log(
      `attempt checkoutFromSpot for user ${userID} at location ${coordinateToString(
        location
      )}`
    );
    try {
      ...
      if (previousPinCheckedInUserIds.indexOf(userID) < 0) {
        throw new Error(
          `user was not checked into spot ${coordinateToString(location)}.`
        );
      }

      // update the pin's checkout info
      previousPinActivity.checkedInUserIds = previousPinCheckedInUserIds.filter(
        (id) => id !== userID
      );
      previousPinActivity.activeUsers--;
      const editPinActivityResult = await this.editPinActivity(
        location,
        previousPinActivity
      );
      ...
      throw new Error(
        `checkout from spot failed for user ${userID} at location ${coordinateToString(
          location
        )}: ${error}`
      );
    }
  ...
  async checkIntoSpot(
    ...
    console.log(
      `attempt checkIntoSpot for user ${userID} at location ${coordinateToString(
        location
      )}`
    );
    try {
      ...
      await updateDoc(userDocRef, {
        checkInSpot: location,
        checkOutTime: checkoutDate,
      });
    } catch (error) {
      throw new Error(
        `check into spot failed for user ${userID} at location ${coordinateToString(
          location
        )}: ${error}`
      );
    }
  ...
  async checkoutAllExpiredCheckins(minutesBetweenCheckoutTask: number) {
    ...
    try {
      ...
      let lastCheckoutAllExpiredCheckins: Date = checkInDocSnap
        .data()
        .lastCheckoutAllExpiredCheckins.toDate();
      console.log(
        `last checkoutAllExpiredCheckins: ${lastCheckoutAllExpiredCheckins}`
      );

      lastCheckoutAllExpiredCheckins.setMinutes(
        lastCheckoutAllExpiredCheckins.getMinutes() + minutesBetweenCheckoutTask
      );
      console.log(
        `next checkoutAllExpiredUsers in ${minutesBetweenCheckoutTask} minutes, at ${lastCheckoutAllExpiredCheckins}`
      );
      ...
      if (currentDate.getTime() > lastCheckoutAllExpiredCheckins.getTime()) {
        console.log(
          `running checkoutAllExpiredUsers: ${lastCheckoutAllExpiredCheckins}`
        );
        ...
        users.forEach((user) => {
          if (
            user._checkInSpot &&
            currentDate.getTime() > user._checkOutTime.getTime()
          ) {
            usersWereCheckedOut = true;
            console.log(
              `checking out user: ${
                user._userID
              }, from spot: ${coordinateToString(
                user._checkInSpot
              )}, with checkout time: ${user._checkOutTime}.`
            );
        ...
        console.log(
          `NOT running checkoutAllExpiredUsers: ${lastCheckoutAllExpiredCheckins}`
        );
      }
    } catch (error) {
  ...
  async checkoutAllExpiredCheckinsTask(
    minutesBetweenTasks: number,
    minutesBetweenRunningCheckoutAllExpiredCheckins: number
  ) {
    setInterval(
      () =>
        this.checkoutAllExpiredCheckins(
          minutesBetweenRunningCheckoutAllExpiredCheckins
        ),
      1000 * 60 * minutesBetweenTasks
    );
  }
  ...
  async editPinActivity(
    ...
    try {
      ...
      await updateDoc(pinRef, {
        activity: pinActivityConverter.toFirestore(activity),
      });
    } catch (error) {
      return new DatabaseActionResult(
        false,
        `Failed: could not edit pin activity at coordinate ${coordinateToString(
          coordinate
        )}. ${error}`
      );
    }
    ...
    return new DatabaseActionResult(
      true,
      `Succeeded: pin activity edited at ${coordinateToString(coordinate)}`
    );
  }
